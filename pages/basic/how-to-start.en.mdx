...
In the end, we provide 6 `imageid` names through the combination of VM and Interface `<interface>-<vm>-<os>:<version>`:
...
2. The second part `<vm>`：`instance` refers to starting a new virtual machine instance (cloud host)，while excluding ' 'instance' (more commonly used，omitted) means starting with a new container instance (Docker).
