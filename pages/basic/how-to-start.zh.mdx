...
1. 优先建议用 WebIDE 环境，如果实验/挑战需要 GUI 环境，则需要使用 VNC。
2. 大多数情况下，默认使用非云主机环境，即不包含 `instance`，因为云主机环境的启动时间较长，且云主机环境的成本高，不适合大量的实验/挑战使用。
3. 如果实验/挑战不支持在容器环境中运行，或者需要使用 `instance` 云主机环境。例如 Docker 相关的实验/挑战，无法在 Docker 容器环境中完成，需要用云主机。

你可以通过 [这个课程](https://labex.io/courses/labex-env-testing/) 体验线上环境。
