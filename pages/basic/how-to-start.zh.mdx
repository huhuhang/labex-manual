...
实验环境可以简单理解由两部分组成，虚拟机（VM）和界面（Interface）。

![.github/assets/vm-interface.png]
...
最终，我们通过 VM 和 Interface 的组合 `<interface>-<vm>-<os>:<version>`，提供了 6 个 `imageid` 名称：
...
2. 第二个部分 `<vm>`：`instance` 指启动一个新的虚拟机实例（云主机），不包含 `instance`（更常用，省略）指启动后是一个新的容器实例（Docker）。
3. 第三个部分 <os>:<version>：`ubuntu:2204` 指启动后的操作系统是 Ubuntu 22.04，目前仅支持 Ubuntu 22.04。

#### Backend Services

<details>
<summary>以下部分不常用</summary>
