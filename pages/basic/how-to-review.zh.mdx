...
- **Major Repair** (大修):
  1. lab/challenge 存在结构性问题，用户无法正常学习；
  2. lab/challenge 存在知识性错误（代码/内容），向用户传达了违反事实的方法和理论；
  3. lab/challenge 存在检测脚本问题，用户可以直接通过或者无法通过；
  4. lab/challenge 反复出现的格式问题。
- **Minor Repair** (小修):
  1. 除大修意外的其他小的格式问题，内容问题等。
- **No Repair** (无需修改):
  1. 不需要修改，可以直接 merge。

Review Tag 将会反馈到 [作者能力模型](https://labex-dev.feishu.cn/sheets/shtcnYuVte2FpZTcDvlFT7puLRc?sheet=a5Gn4d)：

1. No Repair 和 Minor Repair 意味着 LabEx 投入的审核精力较少，作者更容易升级；
...
1. 开启测试环境，确保内容可以在环境中正常运行，特别需要注意：
   - lab/challenge 选择了合适的环境，参考 [Backend 配置](how-to-start)
...
6. Review 完成，添加 [Review Tag](#review-tag) 标记当前 PR 的修改类型。
